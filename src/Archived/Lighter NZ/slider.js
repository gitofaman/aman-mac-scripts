function initiateSlidesDevelopment() {
    var sliderWrapper = document.querySelector('#wrapper')
    var slideItems = document.querySelectorAll('.collection-item-4')
    var slideMainNode = document.querySelector('#main-node')
    //creating slide items for each image
    slideItems.forEach(slideItem => {
        var slideMainClone = slideMainNode.cloneNode(true)
        var slideImageSrc = slideItem.querySelector('img').getAttribute('src')
        slideMainClone.removeAttribute('id')
        slideMainClone.querySelector('img').setAttribute('src', slideImageSrc)
        slideMainClone.querySelector('script').remove()
        sliderWrapper.appendChild(slideMainClone)
        slideMainClone.addEventListener('click',()=>{
            slideItem.querySelector('a').click()
        })
    })
}
initiateSlidesDevelopment()
...
var createFunctioningSlider = (slider) => {
    ...
    if(slider.hasAttribute('auto-slide-duration')) {
        ...
        sliderElements.forEach(slideElement=>{
            slideElement.addEventListener('click',()=>{isPaused=true})
        })
    }
};
