...
var desktopScript = () => {
    ...
    var adjustAboutBlockFont = () => {
        if(aBlocks[0].getAttribute('el-state') !== 'active') {
            var paras = aBlocks[0].querySelectorAll('[para]')
            paras.forEach(para=>{
                console.log(aBlocks[0].offsetHeight)
                console.log(aBlocks[0].offsetWidth)
                var fontSizeToUse = (1.56 * aBlocks[0].offsetWidth * aBlocks[0].offsetHeight)/144228
                if(fontSizeToUse < 1.05) {
                    fontSizeToUse = 1.05
                }
                para.style.fontSize = fontSizeToUse + 'em'
            })
        }
    }
...
// var resetEverything = () => {
//     var aBlocks = Array.from(document.querySelectorAll('.v3-block'))
//     aBlocks.forEach(aBlock=>{
//         aBlock.removeAttribute('style'); aBlock.removeAttribute('el-state')
//         var clonedBlock = aBlock.cloneNode(true)
//         aBlock.parentNode.replaceChild(clonedBlock, aBlock);
//     })
// }

var startingWindowWidth = window.innerWidth
if(startingWindowWidth > 991) {
    desktopScript()
} else {
    mobileViewScript()
}
...
window.addEventListener('resize', () => {
    if(startingWindowWidth>991 && window.innerWidth <= 991) {
        refreshPage()
    }
    if(startingWindowWidth < 991 && window.innerWidth>= 991) {
        refreshPage()
