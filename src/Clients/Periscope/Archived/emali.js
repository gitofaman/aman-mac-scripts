...
forms.forEach(form=>{
    ...
    const keywordParam = thisSiteUrl.searchParams.get('keyword')
    const targetParam = thisSiteUrl.searchParams.get('targetid')
    var valueToInput = ''
    ...
    if (!!keywordParam) {
        valueToInput += ` keyword=${keywordParam}`
    }
    if (!!targetParam) {
        valueToInput += ` targetid=${targetParam}`
    }
